...
format-in can be either a control string or a previously compiled format."
  ...
  `(let [format-in# ~format-in
         my-c-c# (var-get (get (ns-interns (the-ns 'clojure.data.pprint))
                               '~'cached-compile))
         my-e-f# (var-get (get (ns-interns (the-ns 'clojure.data.pprint))
                               '~'execute-format))
         my-i-n# (var-get (get (ns-interns (the-ns 'clojure.data.pprint))
                               '~'init-navigator))
...
format-in can be either a control string or a previously compiled format."
  ...
  [format-in]
  `(let [format-in# ~format-in
         cf# (if (string? format-in#) (#'clojure.data.pprint/cached-compile format-in#) format-in#)]
     (fn [& args#]
       (let [navigator# (#'clojure.data.pprint/init-navigator args#)]
         (#'clojure.data.pprint/execute-format cf# navigator#)))))
