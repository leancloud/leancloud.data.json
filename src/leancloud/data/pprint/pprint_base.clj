...
(def ^{:private true} write-option-table
     {;:array            *print-array*
      :base             'leancloud.data.pprint/*print-base*,
      ;;:case             *print-case*,
      :circle           'leancloud.data.pprint/*print-circle*,
      ;;:escape           *print-escape*,
      ;;:gensym           *print-gensym*,
      :length           'clojure.core/*print-length*,
      :level            'clojure.core/*print-level*,
      :lines            'leancloud.data.pprint/*print-lines*,
      :miser-width      'leancloud.data.pprint/*print-miser-width*,
      :dispatch         'leancloud.data.pprint/*print-pprint-dispatch*,
      :pretty           'leancloud.data.pprint/*print-pretty*,
      :radix            'leancloud.data.pprint/*print-radix*,
      :readably         'clojure.core/*print-readably*,
      :right-margin     'leancloud.data.pprint/*print-right-margin*,
      :suppress-namespaces 'leancloud.data.pprint/*print-suppress-namespaces*})
...
leancloud.data.pprint.dispatch.clj."
  {:added "1.2"}
  [function]
...
and :suffix."
  ...
  (let [[options body] (parse-lb-options #{:prefix :per-line-prefix :suffix} args)]
    `(do (if (#'leancloud.data.pprint/level-exceeded) 
           (.write ^java.io.Writer *out* "#")
           (do 
             (push-thread-bindings {#'leancloud.data.pprint/*current-level*
                                    (inc (var-get #'leancloud.data.pprint/*current-level*))
                                    #'leancloud.data.pprint/*current-length* 0})
             (try  
              (#'leancloud.data.pprint/start-block *out*
                           ~(:prefix options) ~(:per-line-prefix options) ~(:suffix options))
              ~@body
              (#'leancloud.data.pprint/end-block *out*)
              (finally 
               (pop-thread-bindings)))))
         nil)))
