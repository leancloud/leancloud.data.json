...
(defn- capitalize-word-writer
  ...
  [^java.io.Writer writer]
  (with-local-vars [last-was-whitespace? true]
    (proxy [java.io.Writer] []
      (close [] (.close writer))
      (flush [] (.flush writer))
      (write
          ...
          (condp = (class x)
            ...
            (let [c (char x)]
              (let [mod-c (if @last-was-whitespace? (Character/toUpperCase (char x)) c)]
                ...
                (var-set last-was-whitespace? (Character/isWhitespace (char x)))))))))))

(defn- init-cap-writer
  "Returns a proxy that wraps writer, capitalizing the first word"
  [^java.io.Writer writer]
  (with-local-vars [capped false]
    (proxy [java.io.Writer] []
      (close [] (.close writer))
      (flush [] (.flush writer))
      (write ([^chars cbuf ^Integer off ^Integer len]
...
format-in can be either a control string or a previously compiled format."
  ...
  `(let [format-in# ~format-in
         my-c-c# (var-get (get (ns-interns (the-ns 'leancloud.data.pprint))
                               '~'cached-compile))
         my-e-f# (var-get (get (ns-interns (the-ns 'leancloud.data.pprint))
                               '~'execute-format))
         my-i-n# (var-get (get (ns-interns (the-ns 'leancloud.data.pprint))
                               '~'init-navigator))
...
format-in can be either a control string or a previously compiled format."
  ...
  [format-in]
  `(let [format-in# ~format-in
         cf# (if (string? format-in#) (#'leancloud.data.pprint/cached-compile format-in#) format-in#)]
     (fn [& args#]
       (let [navigator# (#'leancloud.data.pprint/init-navigator args#)]
         (#'leancloud.data.pprint/execute-format cf# navigator#)))))
